...
public abstract class DBFactory {
  ...
  public void commitSQL() throws Exception {
    DLTool.writeLog(this.getClass()).debug("COMMIT SQL TRANSACTION.");
    conn.commit();
  }
  ...
  public void setAutoCommit(boolean state) throws Exception {
    DLTool.writeLog(this.getClass()).debug("SET AUTOCOMMIT MODE FROM " + conn.getAutoCommit() + " TO " + state + ".");
  ...
  public String setParam(String sql, String param) {
    sql = sql.replaceFirst(DLLang.AT, param.toUpperCase());
    return sql;
  }

  public int[] executeBatch() throws Exception {
    DLTool.writeLog(this.getClass()).debug(pStmt.toString());
  ...
  public String sExecuteQuery() throws Exception {
    ...
    rs = pStmt.executeQuery();
    rs.last();
    String sRow = DLLang.EMPTY;
    rs.beforeFirst();
    while (rs.next()) {
      sRow = rs.getString(1);
    }
    return (sRow == null ? DLLang.EMPTY : sRow);
  }
